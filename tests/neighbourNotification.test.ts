...
// The 17-day neighbour notification, counted the way the practice
...
describe("the neighbour notification period", () => {
  test("an ordinary start is day one, and the period ends 16 days later", () => {
    // 7 September 2026 is a Monday.
    assert.equal(notificationFirstDay("2026-09-07"), "2026-09-07");
    assert.equal(notificationEndDate("2026-09-07"), "2026-09-23");
  });
  ...
  test("a Friday start is counted from the following Tuesday", () => {
    ...
    assert.equal(notificationEndDate("2026-09-04"), "2026-09-24");
  });
  ...
  test("a Thursday start counts as its own day one", () => {
    ...
    assert.equal(notificationEndDate("2026-09-03"), "2026-09-19");
  });

  test("the count is 17 days inclusive of day one", () => {
    // Day one the 7th, day seventeen the 23rd: 17 days, not 18.
    const first = new Date("2026-09-07T00:00:00");
    const last = new Date(notificationEndDate("2026-09-07")! + "T00:00:00");
    assert.equal((last.getTime() - first.getTime()) / 86400000 + 1, 17);
  });
  ...
  test("a full timestamp is read as its date", () => {
    assert.equal(notificationEndDate("2026-09-07T10:30:00.000Z"), "2026-09-23");
  });
});
