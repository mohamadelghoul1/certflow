...
// Generous: an architectural set can be large, and a client who has to
// split a drawing to send it will email it instead, which helps nobody.
...
export function uploadProblem(file: { name: string; size: number }): string | null {
  ...
  if (file.size > MAX_UPLOAD_BYTES) {
    ...
    return `That file is ${mb} MB and the limit is ${MAX_UPLOAD_BYTES / (1024 * 1024)} MB. Send it as a PDF, or split it into parts.`;
  }
  if (file.size === 0) {
