...
export async function getIssuanceEvents(supabase: SupabaseServerClient, firmId: string): Promise<IssuanceEvent[]> {
  ...
  // Independent tables, fetched together — see getAuditEvents.
  const [{ data: jobs }, { data: nocChecklists }, { data: ocRecords }] = await Promise.all([
...
export async function getAuditEvents(supabase: SupabaseServerClient, firmId: string): Promise<AuditEvent[]> {
  ...
  // Four independent tables, asked in one round trip's time rather than
  // four — this is the page's whole wait, so the difference is the page.
  const [{ data: jobs }, { data: modifications }, { data: ocRecords }, { data: inspections }] = await Promise.all([
    excludingDeleted((live) => {
      const query = supabase
        .from("jobs")
